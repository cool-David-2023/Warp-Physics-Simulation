...
def update_from_cache(scene):
    ...
    try:
        ...
        for obj_name, wireframe_obj in config.wireframe_objects.items():
            if wireframe_obj is None:
                continue
            
            # 載入該物件的幀數據
            positions_warp = cache_mgr.load_frame(frame, obj_name)
            
            if positions_warp is not None:
                # 判斷物件類型（Grid 或 Mesh）
                if hasattr(wireframe_obj, 'get') and wireframe_obj.get("warp_tet_type") == "grid":
                    # Grid: 物件本身，不更新（因為只是代表）
                    # 或者創建動態網格更新（複雜）
                    pass
                else:
                    # Mesh: 更新線框網格
                    update_wireframe_mesh(wireframe_obj, positions_warp)
