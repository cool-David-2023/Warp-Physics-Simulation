...
class WARP_OT_Bake(bpy.types.Operator):
    ...
    def _prepare_bake(self, context):
        ...
        # 清除舊快取
        if self.cache_mgr.has_cache():
            print("⚠️ 偵測到舊快取數據，正在清除...")
    ...
    def _simulate_frame(self, context):
        ...
        # 儲存每個物件的位置到獨立檔案
        for obj_name, obj_info in config.cache_info['objects'].items():
            ...
            # 儲存（使用新的 3 參數簽名）
            self.cache_mgr.save_frame(frame, obj_name, obj_positions)
        
        t_save = time.perf_counter()
        
        # 即時更新視窗（每 3 幀更新線框）
        if self._current_frame_idx % 3 == 0:
            context.scene.frame_set(frame)
            # 更新所有線框
            for obj_name, wireframe_obj in config.wireframe_objects.items():
                if wireframe_obj and obj_name in config.cache_info['objects']:
                    obj_info = config.cache_info['objects'][obj_name]
                    particle_offset = obj_info['particle_offset']
                    particle_count = obj_info['particle_count']
                    obj_positions = all_positions[particle_offset:particle_offset + particle_count]
                    
                    from ..blender_integration.mesh_builder import update_wireframe_mesh
                    update_wireframe_mesh(wireframe_obj, obj_positions)
